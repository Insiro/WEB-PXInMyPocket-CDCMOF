<template>
  <div class="flex flex-row h-full">
    <div :key="prod" :class="BoxStyle" />
    <WideFrame class="flex-grow Container" container_class="grid grid-col-1">
      ...
      <Button :class="[BlockClass]" @onClick="onRegistClicked">등록</Button>
    </WideFrame>
    <div :class="BoxStyle" />
  </div>
</template>
...
export default class NewProd extends Vue {
  ...
  async onRegistClicked(): Promise<void> {
    if (!isValidProd(this.prod)) {
      this.toast.error("필드 값을 확인하시오");
      return;
    }
    if (globalState.pageName === "상품 등록") {
      await curItemState.AddProduct();
    } else {
      await curItemState.changeItemMeta();
    }
  }
  //#endregion
}
